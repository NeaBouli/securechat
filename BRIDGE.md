...
## 2026-07-13 [Codex]
### TYPE: PAYMENT / ENTITLEMENT COMPATIBILITY
### STATUS: LOCAL CROSS-REPO VECTOR PASS / PRODUCTION DISABLED

- SecureChat Pro/Elite is recognized by the shared signed VLABS fulfillment server with product-specific delivery, duplicate and revoke handling.
- The Kotlin verifier accepts a canonical token generated by the actual Node entitlement signer and still enforces SecureChat audience, product, device, tier, expiry and signature binding.
- Operational values remain private. No payment, activation, provider request or deployment was performed.
