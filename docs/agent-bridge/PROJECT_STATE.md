...
- The signed consumer binds each supported SecureChat product to its exact tier and rejects mismatches.
...
- A fixed public cross-repository vector generated by the production Node signer passes the SecureChat Kotlin verifier with exact audience, device, product, tier and expiry checks.
- Shared server fulfillment recognizes SecureChat Pro/Elite, while runtime keys, payment E2E, physical-device and legal/provider gates remain closed.
